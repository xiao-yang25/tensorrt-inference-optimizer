...
__global__ void BevPoolKernel(int channels, int n_intervals, int map_size, const float* depth, const float* feat,
  ...
  output[channel_index * map_size + bev_rank] = pooled;
}
...
void LaunchBevPoolKernel(int channels, int n_intervals, int map_size, const float* depth, const float* feat,
  ...
  const int total = channels * n_intervals;
  const int blocks = (total + kThreads - 1) / kThreads;
  BevPoolKernel<<<blocks, kThreads, 0, stream>>>(channels, n_intervals, map_size, depth, feat, ranks_depth,
                                                  ranks_feat, ranks_bev, interval_starts, interval_lengths, output);
}
